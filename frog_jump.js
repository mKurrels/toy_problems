...
var getTimeCross = function (leavesArray, pondWidth, maxJump) {
  var position = 0;
  for (var i = 0; i < leavesArray.length; i++) {
    var leaf = leavesArray[i];
    if (leaf > position && leaf - position <= maxJump) {
      position = leaf;
    }
    if (pondWidth - maxJump <= position) {
      return i;
    }
  }
  return -1;
...
console.log(getTimeCross([1,3,1,4,3,5], 7, 3)); //should be 3